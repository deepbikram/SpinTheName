...
export class SettingService {
  ...
  async initDarkMode() {
    try {
      DarkMode.value = (await this.getSetting('darkMode')).value;
    } catch (e) {
      DarkMode.value = false;
      this.addSetting({ key: 'darkMode', value: false });
    }

    watch(DarkMode, async (newValue) => {
      try {
        const doc = await this.getSetting('darkMode');
        doc.value = newValue;
        await this.updateSetting(doc);
      } catch (e) {
        await this.addSetting({ key: 'darkMode', value: newValue });
      }
    });
  }
